 ...
 */
function makeIcon(rows, r, g, b, colorMap = {}) {
 ...
 */
function makeLine(width, color = 'FFFFFF') {
  return color.repeat(width);
}

/**
 * Color range helper — creates the stops format.
 * Input: [[position, 'RRGGBB'], ...]
 */
function colorRange(min, max, stops) {
  return { min, max, stops };
}

module.exports = { makeIcon, makeLine, colorRange };
